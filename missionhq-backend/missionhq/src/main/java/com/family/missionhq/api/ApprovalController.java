...
/** Parent-side endpoints. Single household in the first cut; parentId is 1 until real parent accounts land. */
...
public class ApprovalController {
    ...
    public Queue queue() {
        var m = completions.findByStatusOrderBySubmittedAtAsc(MissionCompletion.Status.PENDING).stream().map(c -> {
        ...
        var r = rewardRepo.findByStatusOrderByIdAsc(Reward.Status.PENDING).stream().map(x -> {
    ...
    public void approveMission(@PathVariable Long id, @RequestBody(required = false) ApproveMission body) {
        missions.approve(id, PARENT_ID, body != null && body.bonusPoints() != null ? body.bonusPoints() : 0);
    }
    ...
    public void sendBack(@PathVariable Long id, @RequestBody(required = false) SendBack body) {
        missions.sendBack(id, PARENT_ID, body != null ? body.note() : null);
    }
    ...
    public Reward approveReward(@PathVariable Long id, @RequestBody(required = false) ApproveReward body) {
        return rewards.approve(id, body != null ? body.price() : null, body != null ? body.tier() : null);
    }

    @PostMapping("/approvals/rewards/{id}/decline")
    public void declineReward(@PathVariable Long id) { rewards.decline(id); }
    ...
    public void bonus(@PathVariable Long id, @RequestBody Bonus body) {
        var kid = kids.findById(id).orElseThrow();
        ledger.award(kid, body.points(), PointEntry.Type.BONUS, null, body.reason(), PARENT_ID);
    ...
    public void reprice(@PathVariable Long kidId, @PathVariable Long rewardId, @RequestBody Reprice body) { rewards.reprice(rewardId, body.price()); }
    ...
    public void rate(@RequestBody Rate body) {
        var hh = households.findById(1L).orElseThrow();
        hh.setPointsPerCurrencyUnit(body.pointsPerCurrencyUnit());
        households.save(hh);
    }

    @PostMapping("/kids/{id}/pairing-code")
    public Map<String, String> pairingCode(@PathVariable Long id) { return Map.of("pairingCode", devices.createPairingCode(id)); }

    @PostMapping("/redemptions/{id}/fulfil")
    public void fulfil(@PathVariable Long id) { rewards.fulfil(id, PARENT_ID); }
}
